...
async function deleteTransaction(req,res){
    ...
    try {
        ...
        if(!transaction){
            res.status(400).send({message : 'Transaction are not found'});
        }
        res.status(200).send({message : 'transaction are Deleted',transaction});
    } catch (error) { 
        res.status(500).send(error);
    }
} 
...
async function transactionReport(req,res){
    ...
    try {
        if (!startDate || !endDate) {
            return res
                .status(400)
                .send({ error: "Start Date and End Date is Required" });
            } 
